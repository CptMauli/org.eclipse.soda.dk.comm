...
public class CommPortIdentifier {
	...
	/**
	 * Non-Api method. Called when close() method is called on CommPort object.
	 * Required to set port owner to null and propagate CommPortOwnership event.
	 */
	 ...
	 * 
	 * @return boolean <CODE>true</CODE> if the port is owned by some
	 *         application, <CODE>false</CODE> if the port is not owned.
	 */
	 ...
	 * <LI> <CODE>PORT_OWNERSHIP_REQUESTED</CODE>: If the application owns
	 * this port and is willing to give up ownership, then it should call <CODE>close</CODE>
	 * now.
	 * </UL>
	...
	void fireOwnershipEvent(final int eventType) {
		final Enumeration eventListeners = this.listeners.elements();
		while (eventListeners.hasMoreElements()) {
			final CommPortOwnershipListener listener = (CommPortOwnershipListener) eventListeners.nextElement();
	 ...
	 *                is thrown on platforms which do not support this
	 *                functionality.
	 */
